...
Quality problems usually come from mixing API boundaries with workbook logic, mutating original files, or introducing thread-unsafe updates.
...
- manual validation of the Web API flow (start, stop, success, failure) is required when the environment supports it
- some broad `except Exception` blocks remain around cleanup safety; do not spread that pattern into core business logic
...
- Core runtime deps: `pandas>=1.3.0,<3.0.0`, `numpy>=1.20.0,<2.0.0`, `openpyxl>=3.0.0,<4.0.0`, `fastapi>=0.110.0,<1.0.0`, `pydantic>=2.0.0,<3.0.0`, `uvicorn>=0.27.0,<1.0.0`, `appdirs>=1.4.0`
...
### Do not update progress state without the progress manager

Multi-threaded sheet processing must report progress through `src/backend/infrastructure/progress_manager.py::ThreadSafeProgressManager`,
which serializes updates with a lock and forwards callbacks.
...
### Do not move workbook rules into Web API handlers

`src/frontend/web_api.py` maps requests to `ParameterDocument` and calls the application runner, but workbook comparison rules belong in `src/backend/domain/`.
...
The codebase contains broad catches for best-effort cleanup and server shutdown.
...
- Web API boundaries in `src/frontend/`
...
4. manually test the API flow for start/stop/success/failure when the environment supports it
...
- Is the change in the correct layer (`frontend`, `backend/application`, `backend/domain`, `backend/infrastructure`, `shared`, or `tests`)?
- Does workbook preprocessing still operate on a temp copy, not the original source file?
- Are long-running operations using stop checks and thread-safe progress callbacks?
- Does failure handling both log the issue and map to a clear HTTP status?
...
### Example 2: User stop is mapped at the API boundary

`src/frontend/web_api.py` translates `InterruptedError` into an explicit HTTP 409 instead of a generic failure.
...
`src/backend/infrastructure/config_manager.py::update_from_parameters` fills in defaults for missing keys
instead of assuming all saved documents already match the latest schema.
