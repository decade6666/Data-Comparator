...
from ..infrastructure.progress_manager import ThreadSafeProgressManager

# 导入重构后的模块
from .dataframe_utils import reorder_columns_with_update_mark_first
...
def _maybe_gc_collect(threshold_percent=70, log_func=None):
    ...
    try:
        ...
        log(
            f"当前进程内存使用率: {current_process_usage_percent:.2f}% | "
            f"系统总内存使用率: {system_overall_usage_percent:.2f}% "
            f"(GC阈值: {threshold_percent}%)",
            log_func,
        )
        ...
        if current_process_usage_percent >= threshold_percent:
            if log_func:
                log_func(
                    f"内存使用达到 {current_process_usage_percent:.2f}%，执行垃圾回收。"
                )
            gc.collect()
            if log_func:
                log_func(
                    f"垃圾回收完成，当前进程内存使用率: {process.memory_percent():.2f}%。"
                )
    except ImportError:
        if log_func:
            log_func(
                "警告: psutil 库未安装。无法执行基于内存的垃圾回收，"
                "将无条件执行 gc.collect()。请运行 'pip install psutil' 安装。"
            )
        gc.collect()
...
def process_single_sheet_complete(
    ...
    try:
        ...
        old_df_raw = read_single_sheet_from_excel(
            old_path,
            sheet_name,
            anchor_row_num,
            header_row_num,
        ...
        if old_df is None and new_df is None:
            ...
            try:
                if "更新情况（标记）" in output_df.columns:
                    result.added_rows_count = int(
                        (output_df["更新情况（标记）"] == "新增").sum()
                    )
            except Exception:
                result.added_rows_count = 0

            log(f"✅ Sheet [{sheet_name}] 处理完成。", log_func)
            return result
        elif old_df is not None and new_df is None:
            # 如果新文件中不存在但旧文件中存在，标记为缺失Sheet（删除）
            log_func(
                f"ℹ️ 检测到缺失Sheet: [{sheet_name}]，跳过比对"
            )  # 修正日志信息，更简洁
            progress_manager.update_sheet_progress(
                sheet_name, "缺失表单"
            )  # 更新进度状态
            ...
            output_df = process_missing_sheet(
                sheet_name, old_df, config, progress_manager
            )
            ...
            try:
                if "更新情况（标记）" in output_df.columns:
                    result.deleted_rows_count = int(
                        (output_df["更新情况（标记）"] == "删除").sum()
                    )
            except Exception:
        ...
        # 修正：将空的DataFrame情况提升到这里单独处理，确保生成结果
            ...
            old_df is not None and new_df is not None and old_df.empty and new_df.empty
        ):
            ...
            merged_name_to_label["更新情况（标记）"] = (
                "更新情况（标记）"  # 确保标记列的标签
            )

            output_df.attrs["sas_file_names"] = all_original_cols
            output_df.attrs["sas_file_labels"] = [
                merged_name_to_label.get(col, col) for col in all_original_cols
            ]
        ...
        # 下面才是正常比对流程：新旧文件都存在且不为空
            ...
            if (
                hasattr(old_df, "empty")
                and old_df.empty
                and not (hasattr(new_df, "empty") and new_df.empty)
            ):
                log_func(
                    f"ℹ️ Sheet [{sheet_name}] (旧版) 没有数据，新版有数据，视为新增内容。"
                )
                # 使用process_new_sheet生成带有"新增"标记和new_df表头的DataFrame
                output_df = process_new_sheet(
                    sheet_name, new_df, config, progress_manager
                )
                ...
                if hasattr(output_df, "attrs"):
                    result.sas_file_names = output_df.attrs.get("sas_file_names", [])
                    result.sas_file_labels = output_df.attrs.get("sas_file_labels", [])
                    result.sas_name_to_label = output_df.attrs.get(
                        "sas_name_to_label", {}
                    )
                # 统计计数
                try:
            ...
            # 判断新DF为空但旧DF不为空的情况（视为内容缺失）
                ...
                log_func(
                    f"ℹ️ Sheet [{sheet_name}] (新版) 没有数据，旧版有数据，视为缺失内容。"
                )
                output_df = process_missing_sheet(
                    sheet_name, old_df, config, progress_manager
                )
...
def process_missing_sheet(sheet_name, old_df, config, progress_manager):
    ...
    try:
        ...
        missing_df = reorder_columns_with_update_mark_first(
            missing_df
        )  # 将标记列移到第一列

        # 从DataFrame的attrs中获取SAS列名到标签的映射
        sas_name_to_label = getattr(missing_df, "attrs", {}).get(
            "sas_name_to_label", {}
        )
        ...
        progress_manager.safe_log(
            f"处理缺失Sheet [{sheet_name}] 时出错: {str(e)}"
        )  # 记录错误日志
...
def process_new_sheet(sheet_name, new_df, config, progress_manager):
    ...
    """
    try:
        # No copy to reduce memory footprint.
        # This path does not reuse the input DataFrame.
        ...
        progress_manager.safe_log(
            f"处理新增Sheet [{sheet_name}] 时出错: {str(e)}"
        )  # 记录错误日志
        return pd.DataFrame()  # 出错时返回空DataFrame
...
def perform_full_comparison(
    ...
    try:
        ...
        # 创建锚点列(_ANCHOR)用于行匹配
        new_df = create_anchor_by_sas_names(
            new_df, key_cols, progress_manager.safe_log, sheet_name
        )
        old_df = create_anchor_by_sas_names(
            old_df, key_cols, progress_manager.safe_log, sheet_name
        )
        ...
        old_original_cols_names = set(
            getattr(old_df, "attrs", {}).get("sas_file_name", list(old_df.columns))
        )
        # 获取新DF的原始SAS列名
        new_original_cols_names = set(
            getattr(new_df, "attrs", {}).get("sas_file_name", list(new_df.columns))
        )

        # 计算新增的列：在新DF有但在旧DF没有的列
        added_cols = [
            col for col in new_original_cols_names if col not in old_original_cols_names
        ]
        # 计算删除的列：在旧DF有但在新DF没有的列
        deleted_cols = [
            col for col in old_original_cols_names if col not in new_original_cols_names
        ]
        ...
        if not dift.empty:
            ...
            row_changes = (
                {}
            )  # 存储每行变化的标记，如 {'row_idx': ['新增', '更新', '删除']}
            diff_dict = (
                {}
            )  # 存储单元格级别的差异详情，格式为 {row_idx: {col_name: flag}}
            ...
            for row_idx, changes in row_changes.items():
                ...
                for col in new_original_cols_names:
                    if (
                        ...
                        and pd.notna(merged_df.loc[row_idx, col])
                        and str(merged_df.loc[row_idx, col]).strip() != ""
                    ):
                ...
                for col in old_original_cols_names:
                    ...
                    if (
                        old_col_name in merged_df.columns
                        and pd.notna(merged_df.loc[row_idx, old_col_name])
                        and str(merged_df.loc[row_idx, old_col_name]).strip() != ""
                    ):
                        is_new_record = False  # 发现旧数据，则不是纯新增记录
                        break

                if is_new_record and not is_deleted_record:
                    merged_df.loc[row_idx, "更新情况（标记）"] = (
                        "新增"  # 标记为新增记录
                    )
                    ...
                    if getattr(config, "merge_deleted_data", True):
                        for col_name in old_original_cols_names:
                            ...
                            if (
                                old_field_name in merged_df.columns
                                and col_name in merged_df.columns
                            ):
                                merged_df.loc[row_idx, col_name] = merged_df.loc[
                                    row_idx, old_field_name
                                ]
                else:
        ...
        if not getattr(config, "merge_deleted_data", True):
            columns_to_drop = list(
                set(
                    ...
                    + [c for c in merged_df.columns if c.endswith("_OLD_")]
                )
            )
        ...
        if not getattr(config, "merge_deleted_data", True) and deleted_cols:
            raw_output_df = raw_output_df.drop(
                columns=[c for c in raw_output_df.columns if c in deleted_cols],
                errors="ignore",
            )

        # 确定Sheet的总体变更类型
        change_type = (
            "data_changed" if has_changes or added_cols or deleted_cols else None
        )
...
def compare_columns_by_sas_names(merged_df, key_sas_names, sheet_name, log_func):
    ...
    if diff_records is None:
        diff_records = []
        log_func("警告: diff_records 初始化为None，已重新初始化为空列表")
    ...
    for col in non_anchor_compare_cols:
        old_col = f"{col}_OLD_"
        if (
            col not in merged_df.columns or old_col not in merged_df.columns
        ):  # 确保新旧列都存在
            continue
    ...
    for col in non_anchor_compare_cols:
        ...
        update_mask = (
            diff_num_num
            | diff_text_text
            | diff_new_text_old_num
            | diff_new_num_old_text
        )
        ...
        if new_addition_mask.any():  # 如果存在新增的行
            addition_indices = new_addition_mask[
                new_addition_mask
            ].index  # 获取新增行的索引
            for idx in addition_indices:
                try:
                    if diff_records is None:  # 防御性编程
                        diff_records = []
                        log_func(
                            "警告: diff_records 初始化为None，已重新初始化为空列表"
                        )

                    diff_records.append(
                        {"row": idx, "col": col, "flag": "新增"}  # 标记为新增
                    )
                except AttributeError as e:
                    log_func(f"错误: diff_records.append失败: {str(e)}")
                    # 重新初始化并重试，处理极端情况
                    diff_records = []
                    diff_records.append({"row": idx, "col": col, "flag": "新增"})
                except Exception as e:
            ...
            for idx in deletion_indices:
                try:
                    if diff_records is None:  # 防御性编程
                        diff_records = []
                        log_func(
                            "警告: diff_records在删除记录append时为None，已重新初始化"
                        )

                    diff_records.append(
                        {"row": idx, "col": col, "flag": "删除"}  # 标记为删除
                    )
                except Exception as e:
                    log_func(f"删除记录append时出错: {str(e)}")
                    if diff_records is None:
                        diff_records = []
                        diff_records.append({"row": idx, "col": col, "flag": "删除"})
            ...
            for idx in update_indices:
                try:
                    if diff_records is None:  # 防御性编程
                        diff_records = []
                        log_func(
                            "警告: diff_records在更新记录append时为None，已重新初始化"
                        )

                    diff_records.append(
                        {"row": idx, "col": col, "flag": "更新"}  # 标记为更新
                    )
                except Exception as e:
                    log_func(f"更新记录append时出错: {str(e)}")
                    if diff_records is None:
                        diff_records = []
                        diff_records.append({"row": idx, "col": col, "flag": "更新"})
    ...
    try:
        if diff_records and len(diff_records) > 0:
            ...
            dift = pd.DataFrame(
                columns=["row", "col", "flag"]
            )  # 如果没有差异，返回空的DataFrame
            log_func(f"   ℹ️ Sheet [{sheet_name}] 未发现差异记录。")
    except Exception as e:
        log_func(f"❌ 创建差异记录DataFrame时出错: {str(e)}")
        dift = pd.DataFrame(
            columns=["row", "col", "flag"]
        )  # 确保在出错时也返回空DataFrame结构

    return dift


def process_edc_multithreaded(
    old_path,
    new_path,
    output_path,
    log_func,
    ...
    try:
        ...
        if excluded_sheets_found:
            log(
                f"⏭️ 跳过 {len(excluded_sheets_found)} 个表单"
                f"（在排除列表中）: {', '.join(excluded_sheets_found)}",
                log_func,
            )
        ...
        if unmatched_excludes:
            log(
                f"ℹ️ 排除列表中存在文件中未找到的表单: {', '.join(unmatched_excludes)}",
                log_func,
            )

        sheets_to_process = [
            sheet for sheet in all_sheets if sheet not in exclude_sheets
        ]  # 排除不需要处理的Sheet
        ...
        progress_manager = ThreadSafeProgressManager(
            len(sheets_to_process), progress_func, log_func
        )
        ...
        update_progress(
            "开始处理表单...", 20, progress_func=progress_func
        )  # 更新GUI进度
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in concurrent.futures.as_completed(future_to_sheet):
                ...
                try:
                    ...
                    if (
                        result.change_type is None
                        and result.df is not None
                        and result.df.empty
                        and not result.differences
                    ):
                        log(
                            f"   ⏩ sheet[{result.sheet_name}]新旧版本均为空且无差异，跳过合并。)",
                            log_func,
                        )  # 调整消息，更清晰
                        continue  # 跳过不写入
                    ...
                    if result.df is None:
                        log_func(
                            f"⚠️ Sheet [{result.sheet_name}] 没有处理结果DataFrame，跳过合并。"
                        )
                    ...
                    if not result.df.empty:
                        for r_idx, r in enumerate(
                            dataframe_to_rows(result.df, index=False, header=False)
                        ):
                            target_ws.append(r)
                            if (
                                r_idx % 5000 == 0
                            ):  # 每5000行执行一次垃圾回收，降低内存峰值
                                # 将 gc.collect() 替换为条件垃圾回收
                                _maybe_gc_collect(
                                    threshold_percent=70, log_func=log_func
                                )
                                # 同步检查是否需要停止
                        ...
                        # 为空数据且仅有表头的表单添加日志
                        log_func(
                            f"ℹ️ Sheet [{result.sheet_name}] 数据为空，仅写入表头。"
                        )
                    ...
                    target_ws._sas_name_to_label = (
                        result.sas_name_to_label
                    )  # 确保列名到标签的映射被传递
                    # 记录行级计数，供汇总表使用
                    try:
                        target_ws._updated_rows_count = int(
                            getattr(result, "updated_rows_count", 0) or 0
                        )
                        target_ws._deleted_rows_count = int(
                            getattr(result, "deleted_rows_count", 0) or 0
                        )
                        target_ws._added_rows_count = int(
                            getattr(result, "added_rows_count", 0) or 0
                        )
                    except Exception:
                    ...
                    if result.change_type == "missing":
                        target_ws.sheet_properties.tabColor = (
                            config.missing_sheet_tab_fill.start_color
                        )  # 缺失Sheet颜色
                    elif result.change_type == "new":
                        target_ws.sheet_properties.tabColor = (
                            config.new_sheet_tab_fill.start_color
                        )  # 新增Sheet颜色
                    elif result.change_type == "data_changed":
                        target_ws.sheet_properties.tabColor = (
                            config.highlight_fill.start_color
                        )  # 数据有变化Sheet颜色

                    log(f"✅ sheet[ {result.sheet_name}]已完成写入", log_func)
        ...
        raise RuntimeError(
            error_msg
        ) from executor_error  # 抛出运行时错误，表示致命问题
                ...
                if (updated_cnt + deleted_cnt + added_cnt) == 0:
                    ...
                    if update_mark_col_idx is not None:
                        for row in ws.iter_rows(
                            min_row=2,
                            max_row=ws.max_row,
                            min_col=update_mark_col_idx,
                            max_col=update_mark_col_idx,
                        ):
                            cell = row[0]
                            v = (
    ...
    try:
        ...
        update_progress(
            "文件保存完成", 100, progress_func=progress_func
        )  # 更新最终进度
        log_func("✅ 文件保存完成")
...
def enable_filter_border_header_all_sheets(wb, output_path):
    ...
    default_header_fill = PatternFill(
        start_color="A6C9EC", end_color="A6C9EC", fill_type="solid"
    )
