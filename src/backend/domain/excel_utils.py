...
def replace_worksheet_headers(worksheet: Worksheet, new_headers: list) -> Worksheet:
    ...
    if len(new_headers) != worksheet.max_column:
        ...
        log(
            f"警告: 新表头数量({len(new_headers)})与工作表列数"
            f"({worksheet.max_column})不匹配。将尝试用新表头覆盖现有表头。",
            None,
        )
        # 调整 new_headers 以匹配现有列数，如果需要则用空字符串填充
        if len(new_headers) < worksheet.max_column:
            new_headers.extend(
                [""] * (worksheet.max_column - len(new_headers))
            )  # 填充空字符串
        else:  # 如果新表头更长，则截断
...
def apply_highlight_to_worksheet(
    ws,
    config,
    sheet_type=None,
    diff_info=None,
    ...
    # 2. Sheet标签颜色已在process_edc_multithreaded中处理，此处无需重复处理。
