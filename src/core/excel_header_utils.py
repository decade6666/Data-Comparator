...
from ..utils.stop_flag_utils import check_stop_frequently

def read_single_sheet_from_excel(file_path: str, sheet_name: str, anchor_row_num: int, header_row_num: int, log_func, cols_to_drop: list = None):
    """
    ...
    try:
        ...
        for r_idx, row in enumerate(ws.iter_rows(min_row=1, max_row=header_rows_to_read, values_only=True)): # 从第1行开始读取到指定表头行
            if r_idx % 50 == 0:
                check_stop_frequently(log_func)
            header_values_list.append([_normalize_value(value) for value in row]) # 提取单元格值

        # 根据索引获取SASFieldLabel和SASFieldName
        if label_row_idx > 0 and label_row_idx <= len(header_values_list):
            sas_field_label = [str(val) if val is not None else '' for val in header_values_list[label_row_idx - 1]]
        
        if name_row_idx > 0 and name_row_idx <= len(header_values_list):
            sas_field_name = [str(val) if val is not None else '' for val in header_values_list[name_row_idx - 1]]
            # 检测锚点行是否存在重复列名
            non_empty_names = [n.strip() for n in sas_field_name if isinstance(n, str) and n.strip()]
        ...
        # 从数据开始行读取（表头行之后）
        for row_idx_absolute, row in enumerate(ws.iter_rows(min_row=header_rows_to_read + 1, values_only=True), start=header_rows_to_read + 1):
            if (row_idx_absolute - (header_rows_to_read + 1)) % 100 == 0:
                check_stop_frequently(log_func)
            row_values = [_normalize_value(value) for value in row] # 获取当前行的所有单元格值
            
            # 检查是否是空行（所有值都为None或空字符串），如果是则停止读取
            if all(v is None or (isinstance(v, str) and not v.strip()) for v in row_values):
                log_func(f"检测到Sheet [{sheet_name}] 在第 {row_idx_absolute} 行遇到空行，停止读取。")
                break 

            data_rows.append(row_values) # 添加数据行
            
            if len(data_rows) > max_rows_to_read: # 如果读取行数超过限制，强制停止
                log_func(f"⚠️ Sheet [{sheet_name}] 读取行数超过 {max_rows_to_read}，强制停止读取。")
                break
        ...
        if not sas_field_name: # 如果没有定义SASFieldName行，则使用SASFieldLabel作为列名
        ...
        if not sas_field_name:
            ...
            if data_rows:
                ...
                sas_field_name = [f"Unnamed_{i}" for i in range(num_cols)] # 生成Unnamed列名
            else:
                sas_field_name = [] # 仍然为空
        ...
        if data_rows:
            ...
            if len(sas_field_name) < actual_num_cols:
                sas_field_name.extend([f"Unnamed_{i}" for i in range(len(sas_field_name), actual_num_cols)]) # 补充Unnamed列名
            if len(sas_field_label) < actual_num_cols:
                sas_field_label.extend([f"Unnamed_{i}" for i in range(len(sas_field_label), actual_num_cols)]) # 补充空标签
        ...
        if cols_to_drop:
            ...
            new_sas_field_name = []
            new_sas_field_label = []
            name_to_label_map = {name: label for name, label in zip(sas_field_name, sas_field_label)}
            
            for col in df.columns: # 遍历删除后的当前列名
                new_sas_field_name.append(col)
                new_sas_field_label.append(name_to_label_map.get(col, col)) # 从原始映射中获取标签
        ...
        df.attrs['sas_file_label'] = sas_field_label[:len(df.columns)] # 存储对应的列标签
        ...
        sas_name_to_label['更新情况（标记）'] = '更新情况（标记）' # 确保"更新情况（标记）"列的映射存在
