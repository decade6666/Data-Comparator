...
class ParameterManager:
    ...
    def __init__(self, main_app_instance) -> None:
        ...
        self.current_config_name: str = BUILTIN_TEMPLATE_CIMS
        self.repository = JsonParameterRepository()

        self._ensure_config_directory()  # 确保配置目录存在
        # 新增：确保内置模板存在
        self._ensure_builtin_templates()
        self.parameters: ParameterDocument = (
    ...
    def get_configs_dir(self) -> str:
        """获取配置文件的存放目录。"""
        return self.repository.get_configs_dir()

    def get_config_path(self, config_name: str) -> str:
        """获取指定配置名称的完整文件路径。"""
        return self.repository.get_config_path(config_name)

    def _ensure_config_directory(self) -> None:
        """确保配置目录存在。"""
        self.repository.ensure_config_directory()

    def _load_initial_config(self) -> ParameterDocument:
        """加载初始配置：优先加载CIMS模板配置。"""
        cims_path = self.get_config_path(BUILTIN_TEMPLATE_CIMS)
        try:
            loaded_params = self.repository.load_document(BUILTIN_TEMPLATE_CIMS)
    ...
    def load_config(self, config_name: str) -> bool:
        """加载指定名称的配置。"""
        try:
            loaded = self.repository.load_document(config_name)
            if loaded is None:
    ...
    def save_config_as(
        ...
        if config_name in (BUILTIN_TEMPLATE_CIMS, BUILTIN_TEMPLATE_TM):
            ...
            return False
        try:
            self.repository.save_document(config_name, parameters_to_save)
            self.current_config_name = config_name
    ...
    def delete_config(self, config_name: str) -> bool:
        ...
        if config_name in (BUILTIN_TEMPLATE_CIMS, BUILTIN_TEMPLATE_TM):
            ...
            return False
        try:
            deleted = self.repository.delete_document(config_name)
            if deleted:
    ...
    def _ensure_builtin_templates(self):
        templates: Dict[str, ParameterDocument] = {
        ...
        try:
            self.repository.ensure_builtin_templates(templates)
        except Exception as e:
            log(f"确保内置模板存在时出错: {e}", None)

    def load_parameters(self) -> ParameterDocument:
    ...
    def _get_original_template_params(self) -> Optional[ParameterDocument]:
    ...
    def _get_builtin_template_params(
        self, template_name: str
    ) -> Optional[ParameterDocument]:
        """获取指定内置模板的原始参数。"""
    ...
    def create_common_cols_page(self, notebook):
        ...
        self.common_cols_list = tk.Listbox(
        ...
        ttk.Button(
            btn_frame, text="添加", command=lambda: self.add_parameter("common_cols")
        ).pack(side="left", padx=5)
        ttk.Button(
            btn_frame, text="删除", command=lambda: self.delete_parameter("common_cols")
        ).pack(side="left", padx=5)
    ...
    def create_exclude_sheets_page(self, notebook):
        ...
        ttk.Button(
            btn_frame, text="添加", command=lambda: self.add_parameter("exclude_sheets")
        ).pack(side="left", padx=5)
        ttk.Button(
            btn_frame,
            text="删除",
            command=lambda: self.delete_parameter("exclude_sheets"),
        ).pack(side="left", padx=5)
    ...
    def create_default_keys_page(self, notebook):
        ...
        ttk.Button(
            btn_frame, text="添加", command=lambda: self.add_parameter("default_keys")
        ).pack(side="left", padx=5)
        ttk.Button(
            btn_frame,
            text="删除",
            command=lambda: self.delete_parameter("default_keys"),
        ).pack(side="left", padx=5)
    ...
    def create_sheet_key_map_page(self, notebook):
        ...
        ttk.Button(
            btn_frame, text="添加", command=lambda: self.add_sheet_key_map()
        ).pack(side="left", padx=5)
        ttk.Button(
            btn_frame, text="删除", command=lambda: self.delete_sheet_key_map()
        ).pack(side="left", padx=5)

        for sheet, keys in self.parameters["sheet_key_map"].items():
            self.sheet_key_map_list.insert(
                tk.END, f"{sheet}: {', '.join(keys)}"
            )  # 显示Sheet名和对应的锚点列表
    ...
    def add_parameter(self, param_type):
        ...
        ttk.Label(content_frame, text="请输入参数值:", style="Dialog.TLabel").pack(
            pady=5
        )
    ...
    def add_sheet_key_map(self):
        ...
        def save():
            ...
            keys = [
                k.strip() for k in keys_entry.get().split(",") if k.strip()
            ]  # 解析逗号分隔的锚点列表
    ...
    def get_parameters(self) -> ParameterDocument:
        """返回当前所有加载和修改后的参数字典。"""
        return self.parameters

    def _get_default_parameters(self) -> ParameterDocument:
        """返回默认参数字典。"""
        return {
            "old_file_path": "",
            "new_file_path": "",
