...
`src/shared` 提供跨层共享契约和轻量工具，避免 Web/API、应用层和领域层重复定义参数结构、日志回调逻辑。
...
- `ParameterDocument`：贯穿 Web/API、应用层与配置管理的参数文档。
...
- 不应在 `shared` 中引入 pandas、openpyxl、FastAPI 等重依赖，避免跨层耦合。
...
- 共享类型变更影响面大，必须同步 Web/API 模型、配置管理和测试。
- `log` 应保持轻量，不要引入业务逻辑。
...
因为 Web/API、应用层和配置仓储都需要共享同一组字段。集中定义可以减少漂移。
...
至少检查 `frontend/web_api.py`、`backend/application`、`backend/infrastructure/config_manager.py` 和对应测试。
